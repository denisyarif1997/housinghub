            ...
            <div class="rounded-2xl border border-[#E2E8F0] bg-white p-4">
                ...
                @elseif ($payment->status === 'pending')
                    <div class="mt-3 grid grid-cols-3 gap-2">
                        <button wire:click="verify({{ $payment->id }})" wire:confirm="Verifikasi pembayaran ini?" class="flex min-h-[44px] items-center justify-center gap-1 rounded-xl border border-emerald-200 text-[14px] font-semibold text-emerald-700"><i data-lucide="check" class="h-4 w-4"></i> Verifikasi</button>
                    ...
                    <tr class="border-t border-[#E2E8F0]">
                        ...
                        <td class="px-4 py-3 text-right whitespace-nowrap">
                            @if ($payment->status === 'pending')
                                <button wire:click="verify({{ $payment->id }})" wire:confirm="Verifikasi pembayaran ini?" class="font-semibold text-emerald-700">Verifikasi</button>
